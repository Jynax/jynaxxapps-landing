/**
 * ArcadeWordmark.tsx — Arcade-tinted Jynaxx wordmark for the jynaxxapps.com
 * Arcade direction.
 *
 ...
 * `size` and `color`. Accent + echo are pinned to the Arcade mode pair
 * (front: hot pink #FF3D7F, echo: cyan #3DEAFF).
 *
 * Mirrors Wordmark.tsx (Tech) — only the colour constants differ.
 *
 * Usage:
...
// Arcade mode accents — hot pink front, cyan echo.
const ARC_INK    = '#FFFFFF';
const ARC_ACCENT = '#FF3D7F'; // hot pink  (front X)
const ARC_ECHO   = '#3DEAFF'; // cyan      (echo X)

const GEIST = 'Geist, system-ui, sans-serif';
const MONO  = '"JetBrains Mono", ui-monospace, monospace';

// Arcade glow (pink-dominant).
const GLOW = '0 0 12px rgba(255,61,127,0.55), 0 0 22px rgba(255,61,127,0.30)';
  ...
  return (
    <div style={{
      ...
      <div style={{
        position: 'relative',
        bottom: -size * 0.015,
        filter: glow ? 'drop-shadow(0 0 10px rgba(255,61,127,0.55))' : undefined,
      }}>
        <EchoFlourishXX
          height={xxH}
          accent={accent}
...
export function JynaxxAppsLockup({ size = 26, color = ARC_INK, glow = true }: JynaxxAppsLockupProps) {
  return (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: size * 0.5 }}>
      ...
      <span style={{
        ...
        textShadow: glow ? '0 0 6px rgba(255,61,127,0.40)' : undefined,
      }}>Apps</span>
    </div>
  );
}
...
export function StackedMonogram({
  ...
  return (
    <div style={{
      ...
      alignItems: 'center',
      gap: size * 0.04,
      filter: 'drop-shadow(0 0 8px rgba(255,61,127,0.45))',
    }}>
